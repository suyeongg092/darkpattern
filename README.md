...
- **서비스 이름/사이트에서 확인하기 클릭** — mock-services에 있는 그 서비스의 실제 페이지로 이동한다.
  화면을 직접 둘러보며 어떤 다크패턴이 숨어 있는지 확인할 수 있다. (뒤로가기는 대시보드로
  돌아온다.)
- **"AI Agent로 해지하기"** — `agent`가 그 서비스에 접속해 다크패턴을 뚫고 실제로 해지를
  완료하는 과정이 화면 상단에 실시간으로 생중계된다. 스크린캐스트와 함께 오른쪽(또는 아래)
  실행 파이프라인 패널에 단계별 판단 로그와 최종 판정(정상 완료 / 사전 차단 / 위반 탐지)이
  순서대로 표시된다.
- **"공격 시뮬레이션으로 테스트"** — 백엔드를 조작하는 ADI(Agent Deceptive Injection) 공격을
  건 상태로 같은 흐름을 실행한다. 화면은 정상처럼 보여도 agent가 사후 검증으로 조작을
  잡아내는지 확인할 수 있다.
- **PC / 모바일 토글** — 실행 화면 상단의 버튼으로 agent가 보고 있는 뷰포트를 PC 폭과 모바일
  폭 사이에서 바꿔볼 수 있다.
- **지난 기록** — 최근 실행 결과(최대 7일 보관)를 다시 불러와 스크린샷과 로그를 복기할 수
  있다.
