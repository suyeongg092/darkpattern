...
- **서비스 이름 클릭/사이트에서 확인하기**: mock-services에 있는 그 서비스의 실제 페이지로 이동한다.
  화면을 직접 둘러보며 어떤 다크패턴이 숨어 있는지 확인할 수 있다. (뒤로가기는 대시보드로
  돌아온다.)
- **🛡️ 무결성 검증**: `agent`(JS)가 그 서비스에 접속해 다크패턴을 뚫고 실제로 해지를 완료하는
  과정이 화면에 실시간으로 생중계된다. 오른쪽 파이프라인 패널에 단계별 판단 로그와 최종
  판정(정상 완료 / 사전 차단 / 위반 탐지)이 순서대로 표시된다.
- **🔍 다크패턴 우회**: `agent/detector`(Python, team_agent)가 서비스별 하드코딩 없이 화면을
  그때그때 판단해 다크패턴을 우회하며 해지를 시도하는 과정이 같은 방식으로 생중계된다.
  각 단계에서 무엇을 다크패턴으로 판단했는지 태그로 표시된다.
- **공격 시뮬레이션(ADI) 걸고 실행 토글**: 켠 상태로 두 버튼 중 하나를 누르면, 백엔드를
  조작하는 ADI 공격을 건 채로 같은 흐름을 실행한다. 화면은 정상처럼 보여도 실제로 조작이
  일어났는지 사후 검증으로 잡아내는지 확인할 수 있다.
- **지난 기록**: 최근 실행 결과(최대 7일 보관, 무결성 검증·다크패턴 우회 둘 다)를 다시
  불러와 스크린샷과 로그를 복기할 수 있다.
...
flowchart LR
    ...
    subgraph B["agent/detector (Python, team_agent)"]
        direction TB
        B1["① 화면 판단<br/>[data-el] → 다크패턴 여부<br/>(naive/rules/llm_rag)"] --> B2["② 후보 선택<br/>[data-testid] 중 진짜 목표에<br/>부합하는 것 판단"]
        B2 --> B3["③ Playwright<br/>클릭·제출"]
        B3 --> B4["④ 사후 상태<br/>API 재조회"]
    end
...
하는 읽기 전용 호출이다. `agent`와 `agent/detector`는 서로 다른 것을 검증한다 — 전자는
백엔드 위조(ADI) 여부를, 후자는 다크패턴 자체를 판단하고 우회하는지를 검증하며, 둘 다
dashboard에서 실시간 화면(CDP 스크린캐스트)과 함께 실행할 수 있다.
...
```bash
npm install                                             
npm --prefix agent exec playwright install chromium    
pip install -r agent/detector/requirements.txt          
...
구독 목록 카드마다 "🛡️ 무결성 검증"(`agent/src/run.js` 실행) / "🔍 다크패턴 우회"
(`agent/detector`의 `team_agent.run` 실행) 버튼이 있다. 상단의 "공격 시뮬레이션(ADI) 걸고
실행" 토글을 켠 채로 누르면 같은 흐름을 공격 모드로 실행한다. 클릭하면 실행 화면과 단계별
로그가 실시간으로 뜬다. `agent/detector`를 로컬에서 쓰려면 파이썬 의존성도 먼저 설치해야
한다 ([agent/detector/README.md](agent/detector/README.md) 참고).
